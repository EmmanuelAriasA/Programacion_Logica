main = do
    ...
    if ( evalarExpre expreEval 0 (length expreEval - 1) ) then do
            let y = aPosfija expreEval 0 (length expreEval - 1)
            putStr"Notacion polaca: "
            putStrLn y
        else putStrLn "Ingrese una expresion valida"
...
aPosfija expre a b
    | a == b = [expre !! a]
    | ( expre !! ( a + 1 ) ) == '-' = (aPosfija expre ( a + 2 ) ( b - 1 ) ) ++ ['~']
    | otherwise =
        (aPosfija expre ( a + 1 ) ((posOp expre ( a + 1 ) ( b - 1) 0 ) - 1)) ++
        (aPosfija expre ((posOp expre ( a + 1 ) ( b - 1 ) 0) + 1) (b - 1)) ++
        [expre !! (posOp expre ( a + 1 ) ( b - 1 ) 0)]